...
class DemoDataService {
  ...
  private startSimulation() {
    ...
    // REAL-TIME 1-SECOND TELEMETRY TICK (Requirement: values change every second)
  ...
  private stepSimulation() {
    // 1. Solar power: realistic drift between 4.70 and 5.15 kW every second
    const solarDelta = (Math.random() - 0.49) * 0.04;
    let newSolar = parseFloat((this.gridData.solarPower + solarDelta).toFixed(2));
    if (newSolar < 4.70) newSolar = 4.75;
    if (newSolar > 5.20) newSolar = 5.12;

    // 2. Energy today: continuously increments slightly
    const energyDelta = Math.random() < 0.3 ? 0.01 : 0.0;
    const newEnergy = parseFloat((this.gridData.energyToday + energyDelta).toFixed(2));

    // 3. Grid Voltage: fluctuates smoothly around 230V ± 1.5V every second
    const voltDelta = (Math.random() - 0.5) * 0.3;
    let newVoltage = parseFloat((this.gridData.gridVoltage + voltDelta).toFixed(1));
    if (newVoltage < 228.8) newVoltage = 229.4;
    if (newVoltage > 232.0) newVoltage = 231.2;

    // 4. Battery: 80% to 86% charging
    const battShift = Math.random() < 0.08 ? (Math.random() > 0.5 ? 1 : -1) : 0;
    ...
    // Save every 1-second telemetry value into database (Firestore + Local DB)
    databaseService.saveTelemetryTick(this.gridData);

    // Update sensors with live 1s micro-drift
    this.sensors = this.sensors.map((s) => {
      const sTempDelta = (Math.random() - 0.5) * 0.08;
      const sHumDelta = Math.random() < 0.2 ? (Math.random() > 0.5 ? 1 : -1) : 0;
      const sPresDelta = Math.random() < 0.1 ? (Math.random() > 0.5 ? 1 : -1) : 0;
    ...
    // Notify all active React UI listeners
  ...
  public getHistoricalTrend(period: "today" | "7days" | "30days"): {
    ...
    if (period === "today") {
      ...
      return {
        labels,
        power: [1.2, 2.8, 4.1, 4.9, this.gridData.solarPower, 3.4, 1.1],
        energy: [4.2, 12.8, 24.5, 36.1, 41.2, 44.5, this.gridData.energyToday],
        voltage: [229.4, 230.1, 230.8, 231.2, this.gridData.gridVoltage, 230.2, 229.9],
        battery: [65, 70, 76, 80, this.gridData.batteryPercentage, 84, 82],
      };
    } else if (period === "7days") {
      const labels = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
      return {
        labels,
        power: [4.5, 4.7, 4.3, 4.9, 5.1, 4.8, this.gridData.solarPower],
        energy: [41.2, 43.8, 39.5, 46.1, 48.0, 44.9, this.gridData.energyToday],
        voltage: [230.1, 230.5, 229.8, 230.9, 231.0, 230.4, this.gridData.gridVoltage],
        battery: [80, 81, 78, 83, 85, 84, this.gridData.batteryPercentage],
      };
    } else {
      const labels = ["W1", "W2", "W3", "W4"];
      return {
        labels,
        power: [4.4, 4.7, 4.9, this.gridData.solarPower],
        energy: [280, 310, 325, 316 + Math.round(this.gridData.energyToday)],
        voltage: [230.2, 230.6, 230.4, this.gridData.gridVoltage],
        battery: [81, 82, 83, this.gridData.batteryPercentage],
      };
    }
  }
  ...
  public getGridHistory(): GridHistoryPoint[] {
    ...
    return times.map((t, idx) => ({
      time: t,
      voltage: idx === times.length - 1 ? this.gridData.gridVoltage : parseFloat((230.0 + Math.sin(idx) * 1.2).toFixed(1)),
      frequency: idx === times.length - 1 ? (this.gridData.gridFrequency || 50.02) : parseFloat((50.0 + Math.cos(idx) * 0.02).toFixed(2)),
      powerFactor: idx === times.length - 1 ? (this.gridData.powerFactor || 0.96) : parseFloat((0.96 + Math.sin(idx * 0.5) * 0.01).toFixed(2)),
      current: idx === times.length - 1 ? (this.gridData.gridCurrent || 12.4) : parseFloat((12.2 + Math.cos(idx) * 0.4).toFixed(1)),
      status: "normal",
    }));
  }
}
